...
import stylesCss from 'themes/thecircle/styles.css';
...
window.Vaadin['_vaadintheme_thecircle_globalCss'] = window.Vaadin['_vaadintheme_thecircle_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_thecircle_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_thecircle_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_thecircle_globalCss'].push(target);
  }
  if (!document['_vaadintheme_thecircle_componentCss']) {
    
    document['_vaadintheme_thecircle_componentCss'] = true;
  }
